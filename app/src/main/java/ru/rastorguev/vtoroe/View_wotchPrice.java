...
public class View_wotchPrice extends ListActivity implements LoaderCallbacks<Cursor> {
    ...
    private float sumInLentf=0;
    private float sumInAushan=0;
    private float sumInGigant=0;
        ...
        if (cursor.getCount() != 0) {
            ...
            if (cursor.moveToFirst()) // если курсор не пустой
            {
                for (int i = cursor.getCount()-1; i >= 0 ; i--) {
                    ...
                    if(priceInLenta[i].equals(null) || priceInLenta[i].equals("")||
                    priceInAushan[i].equals(null) || priceInAushan[i].equals("")||
                            priceInGigant[i].equals(null) || priceInGigant[i].equals(""))
                    {
                        //priceInLenta[i]!=null & priceInAushan[i]!=null & priceInGigant[i]!=null &
                      //  priceInLenta[i]!="" &
                        sumInLentf+=0;
                        sumInAushan+=0;
                        sumInGigant+=0;
                    ...
                    {
                        sumInLentf+=Float.valueOf(priceInLenta[i]);
                        sumInAushan+=Float.valueOf(priceInAushan[i]);
                        sumInGigant+=Float.valueOf(priceInGigant[i]);
                    }
            ...
            ArrayList<HashMap<String, Object>> data = new ArrayList<HashMap<String, Object>>(items.length+1);
           ...
           String color=dbHelperCat.color(cats2);
            ...
            for (int i = 0; i < items.length; i++) {
                ...
                map.put("Price_in_lenta", priceInLenta[i]);
                map.put("Price_in_aushan", priceInAushan[i]);
                map.put("Price_in_gigant", priceInGigant[i]);
                data.add(map);
            }
            map = new HashMap<String, Object>();
            map.put("Items", "Общая сумма");
            map.put("Price_in_lenta", Float.toString(sumInLentf));
            map.put("Price_in_aushan", Float.toString(sumInAushan));
            map.put("Price_in_gigant", Float.toString(sumInGigant));
            data.add(map);

// Массив имен атрибутов, из которых будут читаться данные
            String[] fromC = {"Items", "Price_in_lenta", "Price_in_aushan", "Price_in_gigant"};
// Массив идентификаторов компонентов, в которые будем вставлять данные
            int[] toC = {R.id.item_name, R.id.priceInLenta, R.id.priceInAushan, R.id.priceInGigant};
            SimpleAdapter adapterc = new SimpleAdapter(this, data, R.layout.view_watch_price_row, fromC, toC);
        ...
        /*    list_category.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View itemClicked, final int position, long id) {
                    ...
                    Toast.makeText(View_wotchPrice.this, items_status_for_chang , Toast.LENGTH_LONG).show();
                 //   dbHelper.changStatus(items[position],items_status_for_chang);
                    ...
                    Intent intent = new Intent(View_wotchPrice.this, View_wotchPrice.class);
